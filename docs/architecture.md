...
Agent adapters integrate at begin, preflight, checkpoint, and finish boundaries. Delivery/acknowledgement is recorded with a monotonic context revision. A checkpoint may cite `basedOnBriefId`; only a material relevant change after that brief creates a stale-assumption finding. Stdio MCP cannot interrupt an active model turn, so urgent delivery reaches the dashboard/person immediately and the agent at its next supported hook/tool call. The first Codex and Claude adapters validate project-scoped MCP plus supported observation surfaces. Optional content-bearing observation is governed by ADR-027 and `agent-activity-sharing.md`; execution-controlling hooks remain disabled.
