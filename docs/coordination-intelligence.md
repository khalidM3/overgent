...
- locally extracted path, language, coarse component, symbol-name, import/package, route, schema/migration, and lockfile signals when available; and
...
- `stale_assumption` — a checkpoint relied on an older brief that has since been materially invalidated for that workstream.
