...
func New(ctx context.Context, wasmModule []byte, names []string, config Config) (*Runtime, error) {
	...
	// The compiler is requested explicitly rather than through
	// wazero.NewRuntimeConfig, which silently falls back to the interpreter on
	// platforms it cannot compile for. ADR-063 requires that a platform without
	// the compiler report no fingerprint and say so, never degrade quietly to a
	// one-second-per-file extractor, so an unsupported platform must fail here.
