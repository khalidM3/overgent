...
func appendPython(v view, found []declaration, outer, inner int, prefix string) []declaration {
	...
	case "function_definition":
		...
		return append(found, declaration{name: prefix + name, kind: kind, raw: pythonDefinitionHeader(v.header(outer, "block"))})
	case "class_definition":
		name, ok := pythonName(v, inner)
		if !ok {
			return found
		}
		found = append(found, declaration{name: name, kind: kindClass, raw: pythonDefinitionHeader(v.header(outer, "block"))})
	...
	case "expression_statement":
		...
		return appendPythonAssignment(v, found, assignment, prefix)
	case "assignment":
		// Newer tree-sitter-python makes expression_statement a supertype, so a
		// module-level assignment appears directly under the module rather than
		// wrapped. Both shapes are handled because the grammar generation is a
		// build-time choice and the rules must not depend on it.
		return appendPythonAssignment(v, found, inner, prefix)
	}
	return found
}

// pythonDefinitionHeader ends a def or class header at the colon that opens its
// body. The parse tree cannot supply this boundary directly: a comment on the
// first body line sits between the colon and the block node, so cutting at the
// block would fold that comment into the signature and make a body-only edit
// move the file contract hash — a false stale-assumption finding on every
// commented Python edit.
//
// The colon is found by scanning rather than by taking the last one, because a
// trailing comment may contain a colon of its own. Colons inside brackets,
// parentheses, braces, or string literals belong to annotations and defaults,
// never to the body.
func pythonDefinitionHeader(header string) string {
	depth := 0
	var quote rune
	escaped := false
	for index, r := range header {
		...
		case r == '\'' || r == '"':
			quote = r
		case r == '#':
			// A comment can only begin after the header is complete, so
			// anything from here on is body text.
			return strings.TrimRight(header[:index], " \t\r\n")
		case r == '(' || r == '[' || r == '{':
			depth++
		case r == ')' || r == ']' || r == '}':
			if depth > 0 {
...
// appendPythonAssignment records a module-level binding as contract surface.
// The assigned value is deliberately excluded, so changing a constant's value
// is not a contract change.
func appendPythonAssignment(v view, found []declaration, assignment int, prefix string) []declaration {
