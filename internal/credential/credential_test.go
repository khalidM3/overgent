...
func TestKeychainRoundTrip(t *testing.T) {
	if os.Getenv("STICKGUY_KEYCHAIN_LIVE") != "1" {
		t.Skip("set STICKGUY_KEYCHAIN_LIVE=1 for disposable Keychain validation")
	}
	account := fmt.Sprintf("validation-%d", time.Now().UnixNano())
	secret := "synthetic-stickguy-validation-secret"
	ctx := context.Background()
