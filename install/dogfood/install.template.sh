#!/bin/sh
# Overgent closed-test installer - UNSIGNED channel.
#
...
if [ "$(uname -s)" != Darwin ]; then
  echo "Overgent is only validated on macOS." >&2
  exit 1
fi
if [ "$(uname -m)" != arm64 ]; then
  echo "Overgent is only validated on Apple Silicon Macs." >&2
  exit 1
fi
...
echo "Downloading Overgent..."
/usr/bin/curl --fail --silent --show-error --location "$origin/Overgent.zip" --output "$temporary/Overgent.zip"
actual_sha="$(/usr/bin/shasum -a 256 "$temporary/Overgent.zip" | /usr/bin/awk '{print $1}')"
if [ "$actual_sha" != "$expected_sha" ]; then
  echo "Checksum verification failed: expected $expected_sha, got $actual_sha" >&2
  exit 1
fi

/usr/bin/ditto -x -k "$temporary/Overgent.zip" "$temporary/extracted"
app="$temporary/extracted/Overgent.app"
if [ ! -d "$app" ] || [ ! -x "$app/Contents/MacOS/overgent-desktop" ]; then
  echo "The downloaded archive is not a usable Overgent app bundle." >&2
  exit 1
fi
...
destination="$destination_directory/Overgent.app"

if [ -d "$destination" ]; then
  echo "Replacing the existing install at $destination ..."
  /usr/bin/osascript -e 'tell application "System Events" to if exists (application process "Overgent") then tell application "Overgent" to quit' >/dev/null 2>&1 || true
...
echo "Opening Overgent - first run walks you through connecting a repository."
