...
origin="${OVERGENT_DOGFOOD_ORIGIN:?set OVERGENT_DOGFOOD_ORIGIN to the public origin}"
convex="${OVERGENT_DOGFOOD_CONVEX:?set OVERGENT_DOGFOOD_CONVEX to the Convex site URL}"
project="${OVERGENT_DOGFOOD_VERCEL_PROJECT:-overgent-dogfood}"
origin="${origin%/}"
convex="${convex%/}"
...
if [ "$deploy" = true ]; then
  vercel_bin="${OVERGENT_DOGFOOD_VERCEL_BIN:-vercel}"
  if ! command -v "$vercel_bin" >/dev/null 2>&1; then
    ...
    echo "Or point at an existing one:  OVERGENT_DOGFOOD_VERCEL_BIN=\$(nvm which 20 >/dev/null 2>&1 && dirname \$(nvm which 20))/vercel" >&2
...
echo "Building Overgent $short against $origin ..."
OVERGENT_PRODUCTION_API_ORIGIN="$origin" \
  OVERGENT_VERSION="v0.1.0-dogfood-$short" \
  OVERGENT_COMMIT="$commit" \
  pnpm --dir "$root" desktop:build >/dev/null
pnpm --dir "$root/apps/dashboard" build >/dev/null

app="$root/apps/desktop/build/bin/Overgent.app"
rm -rf "$staging"
...
cp "$app/Contents/Resources/overgent" "$staging/public/overgent"
ditto -c -k --keepParent "$app" "$staging/public/Overgent.zip"
...
cat > "$staging/vercel.json" <<JSON
{
  ...
  "headers": [
    ...
    { "source": "/overgent", "headers": [{ "key": "content-type", "value": "application/octet-stream" }, { "key": "cache-control", "value": "no-store" }] },
    { "source": "/Overgent.zip", "headers": [{ "key": "content-type", "value": "application/zip" }, { "key": "cache-control", "value": "no-store" }] }
  ]
}
JSON
...
served_app="$(curl -fsSL "$origin/Overgent.zip?cachebust=$$" | shasum -a 256 | awk '{print $1}')"
