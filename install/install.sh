...
if [ "$update_public_key" = '__OVERGENT_UPDATE_PUBLIC_KEY__' ] || [ "$apple_team_id" = '__OVERGENT_APPLE_TEAM_ID__' ]; then
...
/usr/bin/tar -xzf "$temporary/archive.tar.gz" -C "$temporary" overgent
...
/bin/chmod 755 "$temporary/overgent"
/bin/mv "$temporary/overgent" "$destination/overgent"
"$destination/overgent" service install

echo "Overgent installed and its per-user service started."
