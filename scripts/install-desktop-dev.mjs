...
if (process.platform !== "darwin") throw new Error("Stickguy Dev.app is currently macOS-only");
...
const source = path.join(root, "apps", "desktop", "build", "bin", "Stickguy Dev.app");
const applications = path.join(os.homedir(), "Applications");
const target = path.join(applications, "Stickguy Dev.app");
const stage = path.join(applications, ".Stickguy Dev.app.installing");
const backup = path.join(applications, ".Stickguy Dev.app.previous");
