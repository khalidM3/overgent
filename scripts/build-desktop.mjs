...
if (!development) {
  ...
  const binarySource = path.join(backendSource, "convex-local-backend");
...
`, { mode: 0o644 });
  const backendSign = spawnSync("codesign", [...signArguments, "--entitlements", entitlements, path.join(backendDestination, "convex-local-backend")], { stdio: "inherit" });
