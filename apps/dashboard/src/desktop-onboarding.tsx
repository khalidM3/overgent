...
const emptyRequest: EnrollmentRequest = { repositoryRoot: "", projectLabel: "", deviceLabel: "This Mac", joinCode: "", enableCodex: true, enableClaude: true };
  ...
  const submit = async () => {
    ...
    try {
      const availableRequest = {
        ...request,
        enableCodex: request.enableCodex && Boolean(state?.adapters.find((adapter) => adapter.name === "Codex")?.installed),
        enableClaude: request.enableClaude && Boolean(state?.adapters.find((adapter) => adapter.name === "Claude Code")?.installed),
      };
      const result = mode === "create" ? await api.createProject(availableRequest) : await api.joinProject(availableRequest);
