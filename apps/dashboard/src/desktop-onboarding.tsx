...
export function DesktopOnboarding({ api = nativeOnboarding, navigate = (url) => window.location.assign(url) }: { api?: NativeOnboarding; navigate?: (url: string) => void }) {
  ...
  // A `stickguy://new-project` deep link from the hosted Project view lands
  ...
  if (state?.enrolled && addProject) {
    return <NewProjectScreen api={api} displayName="" navigate={navigate} backLabel={state.repositoryLabel || "Stickguy"} onBack={() => setAddProject(false)} />;
  }
    ...
    return <main className="onboarding-shell">
      ...
      <section className="onboarding-card">
        ...
        <p className="onboarding-lede">{revoked
          ? "A Project owner removed this device from Stickguy, so it can no longer read or publish coordination facts. To use Stickguy again, ask an owner for a new invite and reconnect this Mac."
        ...
        <div className="connection-line needs-attention">
          ...
          <div>
            ...
            <p>Stickguy never modifies your working tree. Reconnecting clears only this Mac’s stored credential and its list of Projects.</p>
          </div>
        </div>
        {confirmingReset
  ...
  if (state.enrolled && state.credential === "uncertain") {
    return <main className="onboarding-shell">
      ...
      <section className="onboarding-card">
        ...
        <h1>Stickguy could not confirm this Mac’s access.</h1>
...
function AdapterList({ state, onReconnect }: { state: OnboardingState; onReconnect?: (agent: "codex" | "claude") => void }) { return <div className="adapter-list" aria-label="Agent connections">{state.adapters.map((adapter) => <div key={adapter.name} className={`adapter-row ${adapter.binding}`}><span className={adapter.runtimeVerified ? "adapter-dot connected" : adapter.hooksNeedReview ? "adapter-dot attention" : adapter.configured ? "adapter-dot pending" : "adapter-dot"} aria-hidden="true" /><span><strong>{adapter.name}</strong><small>{adapter.runtimeVerified ? `Verified · ${adapter.fidelity}` : adapter.detail || (adapter.installed ? "Detected · not connected yet" : "Not detected · Git fallback active")}</small></span>{adapter.reconnectAllowed && onReconnect && <button className="text-button" onClick={() => onReconnect(adapter.name === "Codex" ? "codex" : "claude")}>Reconnect to this Project</button>}</div>)}</div>; }
