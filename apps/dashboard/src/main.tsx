...
function PersonGroup({ memberName, streams, selected, onSelect }: { memberName: string; streams: Workstream[]; selected: Selection | null; onSelect: (id: string) => void }) {
  ...
  return <section className="person-group" aria-labelledby={`person-${streams[0]?.id}`}><header><span className="person-avatar">{initialsFor(memberName)}</span><div><h3 id={`person-${streams[0]?.id}`}>{memberName}</h3><p><span className={`presence-dot ${presence}`} />{presence === "online" ? "Working now" : presence === "idle" ? "Recently active" : presence === "paused" ? "Sharing paused" : "Offline"}</p></div><span>{streams.length} {streams.length === 1 ? "session" : "sessions"}</span></header><div className="session-list">{streams.map((stream) => <SessionRow key={stream.id} session={stream} selected={selected?.kind === "session" && selected.id === stream.id} onClick={() => onSelect(stream.id)} />)}</div></section>;
}
...
function SessionRow({ session, selected, onClick }: { session: Workstream; selected: boolean; onClick: () => void }) {
  ...
  return <button className={selected ? "session-row selected" : "session-row"} onClick={onClick} aria-pressed={selected} aria-label={`Open ${vendor} session for ${session.memberName}`}><span className={`agent-icon ${session.agent?.vendor ?? "manual"}`}>{session.agent ? <Bot size={16} /> : <Code2 size={16} />}</span><span className="session-copy"><span className="session-title"><strong>{vendor}</strong>{session.agent?.sessionAlias && <code>{session.agent.sessionAlias}</code>}<span className={`session-state ${session.agent?.status ?? session.presence}`}>{session.agent?.status ?? session.presence}</span></span><span className="session-action">{session.outcome}</span><span className="session-meta">{session.agent?.tool && <span><Command size={12} />{session.agent.tool}</span>}{activeSubagents > 0 && <span><Users size={12} />{activeSubagents} {activeSubagents === 1 ? "subagent" : "subagents"}</span>}<span><FileCode2 size={12} />{session.pathCount.toLocaleString()} {session.pathCount === 1 ? "path" : "paths"}</span><span>{session.updatedLabel}</span></span></span><ChevronRight className="row-chevron" size={16} /></button>;
}
...
function SessionInspector({ session }: { session: Workstream }) {
  ...
  return <div className="inspector-content"><header className="inspector-header"><span className={`agent-icon large ${session.agent?.vendor ?? "manual"}`}>{session.agent ? <Bot size={19} /> : <Code2 size={19} />}</span><div><p>{session.memberName}</p><h2>{vendor}</h2>{session.agent?.sessionAlias && <code>{session.agent.sessionAlias}</code>}</div></header><div className="inspector-status"><span className={`presence-dot ${session.presence}`} /><strong>{statusCopy(session)}</strong><span>{session.updatedLabel}</span></div><section className="inspector-section"><h3>Current activity</h3><p className="activity-copy">{session.outcome}</p>{session.agent?.tool && <div className="tool-line"><Command size={14} /><span>Using</span><code>{session.agent.tool}</code></div>}</section><section className="inspector-section"><h3>Files and paths</h3>{session.paths.length > 0 ? <ul className="path-list">{session.paths.map((path) => <li key={path}><FileCode2 size={14} /><code>{path}</code></li>)}</ul> : <p className="muted-copy">No safe paths have been reported yet.</p>}{session.pathCount > session.paths.length && <p className="muted-copy">{(session.pathCount - session.paths.length).toLocaleString()} additional paths summarized.</p>}</section>{session.agent && <section className="inspector-section"><h3>Subagents <span>{activeSubagents.length}</span></h3>{activeSubagents.length > 0 ? <ul className="subagent-list">{activeSubagents.map((agent) => <li key={agent.alias}><span className="subagent-icon"><Bot size={13} /></span><span><strong>{agent.agentType || "Subagent"}</strong><code>{agent.alias}</code></span><span className={`session-state ${agent.status}`}>{agent.status}</span></li>)}</ul> : <p className="muted-copy">No active subagents.</p>}</section>}{session.largeChange && <section className="inspector-section large-change-detail"><h3>Large change</h3><strong>{session.largeChange.pathCount.toLocaleString()} paths</strong><p>{session.largeChange.summary}</p><small>Manifest revision {session.largeChange.revision}. Size alone does not imply risk.</small></section>}<dl className="session-facts"><div><dt>Connection</dt><dd>{fidelityLabel(session.fidelity)}</dd></div><div><dt>Presence</dt><dd>{session.presence}</dd></div></dl></div>;
}
