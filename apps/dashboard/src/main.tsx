...
function WorkstreamCard({ workstream }: { workstream: ProjectSnapshot["workstreams"][number] }) {
  return <article className="workstream-card"><div className="member-column"><span className={`avatar ${workstream.agent ? "agent-avatar" : ""}`}>{workstream.agent?.vendor === "codex" ? "CX" : workstream.agent?.vendor === "claude" ? "CL" : workstream.initials}</span><span className={`presence-dot ${workstream.presence}`} aria-label={`${workstream.presence} presence`} /></div><div className="workstream-body"><div className="workstream-heading"><div><h3>{workstream.title}</h3><p>{workstream.memberName} · updated {workstream.updatedLabel}</p></div><span className={`fidelity-chip ${workstream.fidelity}`}>{fidelityLabel(workstream.fidelity)}</span></div><p className="outcome">{workstream.outcome}</p>{workstream.agent && <div className="agent-session-meta"><span className={`agent-status ${workstream.agent.status ?? "idle"}`}>{workstream.agent.status ?? "idle"}</span>{workstream.agent.tool && <span>Tool: {workstream.agent.tool}</span>}<span>{workstream.agent.subagents.filter((agent) => agent.status !== "done").length} active subagents</span></div>}<div className="path-row"><span>{workstream.pathCount.toLocaleString()} paths</span>{workstream.paths.map((path) => <code key={path}>{path}</code>)}</div>{workstream.largeChange && <div className="large-change"><span aria-hidden="true">↗</span><div><strong>Large change · {workstream.largeChange.pathCount.toLocaleString()} paths</strong><p>{workstream.largeChange.summary}</p><small>Manifest revision {workstream.largeChange.revision} · broad size does not imply severity</small></div></div>}</div></article>;
}
...
function FindingDetail({ finding, workstreams, disabled, onState, onFeedback }: { finding: Finding; workstreams: ProjectSnapshot["workstreams"]; disabled: boolean; onState: (state: FindingState) => void; onFeedback: (value: FindingFeedback) => Promise<void> }) {
