...
const RECOVERABLE_TICKET_FAILURE: ActivationFailureCopy = {
  ...
  detail: "Return to the Stickguy app and choose Open live Project again. Your Project, repository, and local observations are unchanged.",
};
...
function activationFailureCopy(code: string): ActivationFailureCopy {
  ...
  if (["unauthorized", "credential_revoked", "forbidden"].includes(code)) return {
    ...
    detail: "Return to the Stickguy app to check this Mac’s connection. Your repository and local work were not changed.",
  };
  if (code === "rate_limited") return {
    title: "Stickguy needs a moment before opening the Project.",
    detail: "Return to the app, wait briefly, and choose Open live Project again. Local observation continues while you wait.",
  };
  return {
    title: "Stickguy could not open the live Project.",
...
export function activationFailureResponse(code: string, status: number, development: boolean): Response {
  ...
  const target = development ? "stickguy-dev://open" : "stickguy://open";
  const html = `<!doctype html>
<html lang="en"><meta charset="utf-8"><meta name="referrer" content="no-referrer">
<meta name="viewport" content="width=device-width,initial-scale=1"><title>Stickguy could not open the Project</title>
<style>
...
</style><main><h1>${copy.title}</h1><p>${copy.detail}</p><a href="${target}">Return to Stickguy</a></main></html>`;
